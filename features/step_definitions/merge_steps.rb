...
Then /^the first article contains the text of both$/ do
  ...
  # FIXME 'should contain' is not an existing expectation
...
Then /^I see the text of the "(.*?)" $/ do |article|
  pending # code to verify each article
end
...
Then /^I don\'t see a merge form$/ do
  ...
  pending # but it does not have the merge partial
end
